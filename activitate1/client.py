...
while True:
    ...
    if comanda == 'EXIT':
        ...
        raspuns = "ERROR Connect to publish messages "
        if not este_conectat:
            print(raspuns)
            continue

        if len(parti) < 2 or not parti[1].strip():
            print("LOCAL ERROR: You must write a text after publish ")
            continue
        ...
        raspuns = "ERROR Connect to delete messages "
        if not este_conectat:
            print(raspuns)
            continue

        if len(parti) < 2:
            print("LOCAL ERROR: Missing id ")
            continue

        try:
            id = int(parti[1])
        except ValueError:
            raspuns = "LOCAL ERROR Id must be an integer "
            print(raspuns)
        ...
        raspuns = "ERROR Connect to list messages "
        if not este_conectat:
            print(raspuns)
            continue
